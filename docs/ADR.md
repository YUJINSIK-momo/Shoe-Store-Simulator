...
## ADR-013: 실제 신발은 CC-BY GLB(부위 노드 분리) — 웹 로드, 네이티브 폴백

- **결정**: 웹 미리보기에 실제 신발 모델 적용. 'Sneakers' by Poly by Google(CC-BY)을 OBJ→GLB(obj2gltf) 변환해 `assets/models/sneakers.glb`로 번들. `@react-three/drei useGLTF` + `expo-asset`로 로드하고, **노드 이름으로 부위별 색** 적용(converse→갑피, laces→끈, tongue→안창, rivet→로고). 네이티브는 절차적 `Shoe3D`로 폴백.
- **이유**: 사실적 + 부위별 둘 다 만족하는 모델이 필요했는데, 이 모델은 부위가 **오브젝트(노드)로 분리**돼 있어 이름 매핑으로 부위별 색이 가능. 단일 머티리얼 사실적 모델(adidas FBX 등)은 부위별 불가라 탈락. 네이티브 GLB 로딩(expo-asset 비동기)은 복잡도가 커 웹 우선.
- **대안**: adidas FBX(단일 머티리얼, 부위별 X), 절차적 저폴리 유지(사실성 낮음)
- **트레이드오프**: 저폴리(컨버스풍)라 실사까진 아님. 밑창은 본체에 융합돼 별도 색 불가(4/5 부위). 웹/네이티브 외형 불일치(네이티브=절차적). CC-BY 출처표기 필요(앱에 표기함). 번들 7MB대(three+drei).
- **결정일**: 2026-06-07
